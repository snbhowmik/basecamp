...
`docker-compose.yml`'s `frontend` service bind-mounts `./app/dist`. If that path doesn't exist when the stack starts, **Docker creates it as `root`**, and the next `npm run build` dies with `EACCES: permission denied, copyfile ... -> app/dist/favicon.ico` — an ownership problem wearing a Vite stack trace. Building first means the directory already exists, owned by you, and Docker just mounts it.

If you've already hit it, delete the root-owned directory and rebuild — `chown` works too, but removing it is cleaner since nothing in there is worth keeping:

```bash
sudo rm -rf app/dist && (cd app && npm run build) && docker compose restart frontend
```

This has caught two consecutive fresh deploys (2026-08-17 and 2026-08-18), which is why it is now the first line of this step rather than a troubleshooting entry.
...
| Mailer logs `sent invite to …` and `invite_email_sent_at` is set, but no mail arrives | The worker's job ended successfully at "handed to the relay" — the failure is downstream. Check the MTA, not Basecamp: `docker logs mailcowdockerized-postfix-mailcow-1 --since 24h \| grep <recipient>`. A `status=bounced` line carries the receiving server's verbatim reason. |
| Recipient bounces with `554 5.7.1 ... detected as Spam` **in reply to RCPT TO** | Rejected *before the body was transmitted*, so it is not content filtering — no template change can fix it. It's a reputation judgment on the sending IP. Check the IP against Spamhaus (`https://check.spamhaus.org/`); a `127.0.0.4` answer from `zen.spamhaus.org` is an XBL listing, which on a VPS is frequently inherited from the IP's previous tenant. Verify the relay is not open (`swaks --to test@example.com --from spoof@example.com --server <host>:25` must be **rejected**) before requesting delisting, or the listing returns. Hit on 2026-08-18 against `152.53.18.209`. |
| Want to confirm the mail path without involving Basecamp | `docker logs mailcowdockerized-rspamd-mailcow-1 --since 24h \| grep <recipient>` — a `no action` verdict with a negative score and `DKIM_SIGNED` means the outbound side is clean and the problem is purely the receiving end. |
