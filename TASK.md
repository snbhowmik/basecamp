...
- [x] **Delivery verified end to end 2026-08-19** — invite emails accepted by both Zoho (`250 Message received`) and Gmail, and the `/invite/<token>` links resolve and render the accept screen. rspamd scores the outbound message `-20.10/15.00` with a valid DKIM signature; the sending IP is off Spamhaus XBL (listing expired once detections stopped, 2026-08-18).
- [!] **Both land in spam on first contact.** Not a misconfiguration — SPF, DKIM, DMARC, rDNS and blocklist status are all clean, which is why they are accepted at all. It is an unwarmed domain whose first automated mail carries a login link. Marking not-spam fixes it per recipient; the domain-wide answer is sending volume over time, or an outbound relay with existing reputation before onboarding at scale.
- [~] Historical: delivery was blocked by sender-IP reputation until 2026-08-18. `mx.zoho.in` rejected with `554 5.7.1 ... detected as Spam` **in reply to RCPT TO** — before the body was transmitted, so no template change affects it. Root cause: `152.53.18.209` is on Spamhaus XBL (`zen.spamhaus.org` → `127.0.0.4`), commonly inherited from an IP's previous tenant. Fix is delisting + confirming the relay isn't open; see DEPLOY.md's troubleshooting rows. The copyable invite link remains the working fallback, which is why it was kept.
